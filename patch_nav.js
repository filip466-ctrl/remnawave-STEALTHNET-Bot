const fs = require('fs');
let content = fs.readFileSync('frontend/src/components/layout/dashboard-layout.tsx', 'utf-8');

const old_nav = `const navWithSections: { to: string; label: string; icon: typeof LayoutDashboard; section: string }[] = [
...
const new_nav = `const navWithSections: { to: string; label: string; icon: typeof LayoutDashboard; section: string; category: string }[] = [
...
const old_nav_items = `function NavItems({ onClick }: { onClick?: () => void }) {
...
const new_nav_items = `function NavItems({ onClick }: { onClick?: () => void }) {
  const location = useLocation();
  const admin = useAuth().state.admin;
  const nav = admin
    ? navWithSections.filter((item) => canAccessSection(admin.role, admin.allowedSections, item.section))
