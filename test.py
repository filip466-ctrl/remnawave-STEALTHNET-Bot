...
text = re.sub(r'function GlassCard\(\{[\s\S]*?\}\) \{[\s\S]*?return \([\s\S]*?</motion\.div>\);\n\}', '''function GlassCard({
  children,
  animIndex = 0,
  className = "",
}: {
...
text = re.sub(r'function MetricCard\(\{[\s\S]*?\}\) \{[\s\S]*?return \([\s\S]*?</motion\.div>\);\n\}', '''function MetricCard({
  index = 0,
  icon: Icon,
  label,
  value,
