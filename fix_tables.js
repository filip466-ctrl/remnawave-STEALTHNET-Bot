const fs = require('fs');
let text = fs.readFileSync('frontend/src/pages/analytics.tsx', 'utf-8');
...
// Promo headers
const p1_old = `            <CardHeader className="pb-2 border-b border-white/10 relative">
...
const p1_new = `            <CardHeader className="pb-2 relative pt-4 px-5">
...
const p2_old = `            <CardHeader className="pb-2 border-b border-white/10 relative">
              <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-primary/50 to-transparent opacity-50" />
              <CardTitle className="text-sm font-mono tracking-widest uppercase text-slate-800 dark:text-white">
                <span className="text-primary/50">[</span> Промокоды (топ 10) <span className="text-primary/50">]</span>
              </CardTitle>
            </CardHeader>`;
const p2_new = `            <CardHeader className="pb-2 relative pt-4 px-5">
              <CardTitle className="text-sm font-mono tracking-widest uppercase text-slate-800 dark:text-slate-300">
...
fs.writeFileSync('frontend/src/pages/analytics.tsx', text);
