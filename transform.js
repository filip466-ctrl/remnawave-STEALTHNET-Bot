const fs = require('fs');
let code = fs.readFileSync('frontend/src/pages/analytics.tsx', 'utf-8');

// 1. Replace GlassCard
code = code.replace(/function GlassCard\(\{[\s\S]*?\}\) \{[\s\S]*?return \([\s\S]*?<\/motion\.div>\);\n\}/, `function GlassCard({
  children,
  animIndex = 0,
  className = "",
}: {
...
// 2. Replace MetricCard
code = code.replace(/function MetricCard\(\{[\s\S]*?\}\) \{[\s\S]*?return \([\s\S]*?<\/motion\.div>\);\n\}/, `function MetricCard({
  index = 0,
  icon: Icon,
  label,
  value,
...
// 3. Update ChartCard to remove the decorative border and adjust styles
code = code.replace(/function ChartCard\(\{[\s\S]*?\}\) \{[\s\S]*?return \([\s\S]*?<\/GlassCard>\);\n\}/, `function ChartCard({ title, icon: Icon, children, index = 0 }: { title: string; icon: React.ElementType; children: React.ReactNode; index?: number }) {
  return (
...
// 4. Remove Glow Filters and styles from Recharts elements
// Find `<filter id="glow">...</filter>` and remove it
code = code.replace(/<filter id="glow"[\s\S]*?<\/filter>/g, '');
// Find `<defs>\s*<\/defs>` and remove empty defs
code = code.replace(/<defs>\s*<\/defs>/g, '');
// Remove style={{ filter: "url(#glow)" }}
code = code.replace(/style=\{\{ filter: "url\(#glow\)" \}\}/g, '');

// Save
fs.writeFileSync('frontend/src/pages/analytics.tsx', code);
